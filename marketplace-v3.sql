...
BEGIN
  ...
  SET available_balance = available_balance + p.amount,
      pending_balance = GREATEST(pending_balance - p.amount, 0),
      updated_at = now()
  ...
  VALUES (p.freelancer_id, p.project_id, p.id, 'release', p.amount, 'Project payment released after client approval');
