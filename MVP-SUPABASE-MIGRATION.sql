...
BEGIN
  ...
  v_project_amount := COALESCE(p.project_amount, p.amount);
  IF v_project_amount <= 0 THEN RAISE EXCEPTION 'Invalid project payment amount'; END IF;

  INSERT INTO public.wallets(user_id) VALUES (p.freelancer_id)
    ON CONFLICT (user_id) DO NOTHING;

  UPDATE public.wallets
  SET available_balance = available_balance + v_project_amount,
      pending_balance = GREATEST(pending_balance - v_project_amount, 0),
      updated_at = now()
  WHERE user_id = p.freelancer_id;

  INSERT INTO public.wallet_transactions(user_id, project_id, payment_id, type, amount, description)
  VALUES (p.freelancer_id, p.project_id, p.id, 'release', v_project_amount, 'Project payment released after client approval');

  UPDATE public.payments SET escrow_status = 'released', released_at = now() WHERE id = p.id;
  UPDATE public.projects SET status = 'completed' WHERE id = p.project_id;
END;
$$;
